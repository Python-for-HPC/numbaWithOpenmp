            ...
            for i in range(len(self.tags)):
                ...
                if cur_tag.name in ["QUAL.OMP.MAP.TOFROM", "QUAL.OMP.MAP.TO", "QUAL.OMP.MAP.FROM", "QUAL.OMP.MAP.ALLOC"]:
                    ...
                    if isinstance(cur_tag_typ, types.npytypes.Array):
                        ...
                        # see core/datamodel/models.py
                        loaded_size = lowerer.loadvar(num_elements_var.name)  # Need side-effect?
                        #loaded_op = loaded_size.operands[0]
                        #loaded_pointee = loaded_op.type.pointee
                        ...
                        if isinstance(start_index_var, ir.Var):
                            loaded_size = lowerer.loadvar(start_index_var.name) # Need side-effect?
                            #loaded_op = loaded_size.operands[0]
                            #loaded_pointee = loaded_op.type.pointee
                            #loaded_str = str(loaded_pointee) + " * " + loaded_size._get_reference()
            ...
            if selected_device == 1:
                ...
                class OpenMPCUDACodegen:
                    ...
                    def _get_target_image_toolchain(self, mod, filename_prefix):
                        ...
                        subprocess.run(['ptxas', '-m64', '--gpu-name', self.sm,
                    ...
                    def get_target_image(self, cres):
                        ...
                        if config.OPENMP_DEVICE_TOOLCHAIN >= 1:
                            return self._get_target_image_toolchain(linked_mod, filename_prefix)
                        else:
                            return self._get_target_image_in_memory(linked_mod, filename_prefix)
...
#                    before_start.append(ir.Print([omp_ub_var], None, inst.loc))
    ...
    def teams_distribute_parallel_for_simd_directive(self, args):
        self.some_distribute_directive(args, "TEAMS.DISTRIBUTE.PARALLEL.LOOP.SIMD", 5, has_loop=True)

    def teams_distribute_parallel_for_directive(self, args):
        self.some_distribute_directive(args, "TEAMS.DISTRIBUTE.PARALLEL.LOOP", 4, has_loop=True)
    ...
    def teams_distribute_parallel_for_simd_clause(self, args):
        if config.DEBUG_OPENMP >= 1:
            print("visit teams_distribute_parallel_for_simd_clause", args, type(args), args[0])
            if isinstance(args[0], list):
                print(args[0][0])
        return args[0]
    ...
    def teams_distribute_parallel_for_clause(self, args):
        if config.DEBUG_OPENMP >= 1:
            print("visit teams_distribute_parallel_for_clause", args, type(args), args[0])
            if isinstance(args[0], list):
                print(args[0][0])
        return args[0]
